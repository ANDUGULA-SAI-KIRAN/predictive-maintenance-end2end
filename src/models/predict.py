...
# 1. Point to DagsHub (Best practice: use Environment Variables)

DAGSHUB_URI = os.getenv("MLFLOW_TRACKING_URI", "https://dagshub.com/YOUR_USER/YOUR_REPO.mlflow")
...
class ModelServer:
    ...
    def load_production_model(self, model_name: str = "predictive_maintenance_model"):
        """Loads the 'latest' or 'production' version from Registry"""
        # In industry, we often use 'latest' or a custom alias like '@champion'
        model_uri = f"models:/{model_name}/latest"
        
        # Load as a general pyfunc so it works for both RF and LGBM seamlessly
        self.model = mlflow.pyfunc.load_model(model_uri)
        
        # Fetch run details for the UI
        client = mlflow.tracking.MlflowClient()
        # Extract run_id from the model metadata
        run_id = self.model.metadata.run_id
        run = client.get_run(run_id)
        
        self.threshold = run.data.metrics.get("optimal_threshold", 0.5)
        self.metadata = {
            "Run ID": run_id,
    ...
    def predict(self, input_dict: dict):
        ...
        # Standardize field names (Keeping your mapping)
        field_map = {
            "type": "type", "air_temperature": "air_temp",
        ...
        # REUSE your existing feature engineering
        df_fe = add_engineered_features(df_input)
        
        # Inference
        # Note: pyfunc.predict returns probabilities or labels depending on how it was logged
        prob = self.model.predict(df_fe) 
        # If your model returns a single value, ensure logic handles it:
        if isinstance(prob, (pd.DataFrame, pd.Series)):
            prob = float(prob.values[0])
